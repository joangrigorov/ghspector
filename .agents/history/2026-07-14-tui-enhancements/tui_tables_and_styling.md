...
- **Client-side Log Segmentation**: Added a stateful parser `segmentLogs` that parses raw downloaded logs sequentially. Whenever it encounters a `##[group]` or `##[section]` group marker, it evaluates it against all upcoming chronological steps using a fuzzy name similarity metric weighted by keyword length. If it matches, it transitions to that step, capturing all intermediate raw log output lines (including trailing status lines printed after group closures) under the correct step without overlaps.
