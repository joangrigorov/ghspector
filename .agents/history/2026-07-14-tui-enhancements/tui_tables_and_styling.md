...
- **Client-side Log Segmentation**: Added a parser `segmentLogs` and helper `parseLogTimestamp` that parses the raw downloaded job log file into step boundaries using the chronological execution start time (`StartedAt`) of each step. This handles custom naming, generic command names, or post-run steps flawlessly since it is based on execution timestamps rather than name-string matching.
