...
- **`renderHeader`**: Render header content using new backgrounds with explicit width set to fill the terminal width, replace spacer newlines with colored padding lines to eliminate the margins, add left/right spacer padding to indent text, and append the border line.
